...
% In diesem Skript können mithilfe von DeLaN von Lutter trainierte Modelle
% validiert werden. Da das Modell keine Matrixinvertierung kann, muss das
% in Matlab noch nachträglich gemacht werden. Es muss jedoch kein Scaler
% verwendet werden.
% -------------------------------------------------------------
...
function [dxdt] = ODE_Neral_Network(t, x, net, F_vec, tau_vec)
...
network_name = "20250409_180006_DeLaN_model.onnx";
network_path = fullfile(my_path, '..', 'Training_Models', 'DeLaN_Lutter', 'Saved_Models', network_name);

net = importNetworkFromONNX(network_path, 'InputDataFormats', {'BC'});

% Netz initialisieren
example_input = rand(6, 1);
dl_example_input = dlarray(example_input, 'CB');
net = initialize(net, dl_example_input);

% ODE-Funktion mit Parametern
odefun_2 = @(t, x) ODE_Neral_Network(t, x, net, F_vec, tau_vec);
