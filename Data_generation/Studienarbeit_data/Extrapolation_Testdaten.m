...
% Ordner mit Plot Funktionen hinzufügen
my_path = fileparts(mfilename('fullpath'));
target_folder = fullfile(my_path, '..');
...
if Rob_Model == 1
    % Startpunkte
    q1_0 = random_init(number_runs, 1.5, 2, false); % Bsp. Intervall r [0, 0.5]
    q2_0 = random_init(number_runs, 3/4*pi, 5/4*pi, false);  % Bsp. Intervall phi [0, pi]
    
    % Differenzen für Endpunkte
    delta_q1 = random_init(number_runs, 0.2, 0.5, true); % Bsp. Intervall delta_r [0.2, 0.5]
    delta_q2 = random_init(number_runs, 1/6*pi, 1/4*pi, true); % Bsp. Intervall delta_phi [1/4*pi, 3/4*pi]
...
if savedata == true
    ...
    target_folder = fullfile(my_path, '..', '..', 'Training_Data', 'Studienarbeit_Data');
    ...
    dateiName = 'Extrapolation_Testdaten_Nruns_' + string(number_runs) + '.mat';   
...
plot_1_name = fullfile(plot_path, 'Abbildung_Variation_Samples.pdf');
