...
% Datum:      23.05.2025
% Beschreibung:
% Dies ist eine leicht abgeänderte Version von Data gen V3 friction. hier
% werden lediglich hafteffekte eingeführt, die das reibmodell nicht kennt.
% -------------------------------------------------------------
...
% Anzahl der Haftreibungsphasen
number_stick_runs = 5;
number_base_points = 10;
...
%% Haftreibungseffekte einfügen

% Haftreibungsindizees festlegen
stick_idx = randperm(number_runs, number_stick_runs);

% Zufällige Stellgrößenverläufe generieren
samples_vec = 1:samples_per_run;

% Stützpunkte generieren
sample_base = linspace(0, samples_per_run, number_base_points);
...
for i = stick_idx
    ...
    tau_base = randn(1, number_base_points);    % zufällige Drehmomente
    tau_base = (tau_base - min(tau_base)) / (max(tau_base) - min(tau_base));    % Drehmomente normieren
    
    % Drehmomente auf Intervall 
    taur_base = taur_min + (taur_max - taur_min) * tau_base;
    tauphi_base = tauphi_min + (tauphi_max - tauphi_min) * tau_base;
    
    % Interpolation (spline für Glattheit)
    taur = interp1(sample_base, taur_base, samples_vec, 'spline');
    tauphi = interp1(sample_base, tauphi_base, samples_vec, 'spline');

    % Maxima und Minima überprüfen
    ...
    traj_data(i).q2 = zeros_vec;
    traj_data(i).q2_p = zeros_vec;
    ...
    traj_data(i).C_2 = zeros_vec;

    % Gewichtskräfte
    traj_data(i).g_1 = zeros_vec;
    traj_data(i).g_2 = zeros_vec;

    % Dämpfungsterme
    traj_data(i).fric_1 = taur';
    traj_data(i).fric_2 = tauphi';

    % Eingeprägte Kräfte/Momente 
    traj_data(i).tau_1 = taur';
    traj_data(i).tau_2 = tauphi';
