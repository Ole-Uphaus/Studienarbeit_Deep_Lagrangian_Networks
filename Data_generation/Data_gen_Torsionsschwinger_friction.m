...
% Datum:      02.07.2025
% Beschreibung:
% Dies ist eine Erweiterung des Skriptes zur Erstellung von Trainingsdaten
% für den Torsionsschwinger. Es wird zusätzlich eine Stribeck Reibung
% berücksichtigt, die gleichermaßen auf beide Scheiben einwirkt.
% -------------------------------------------------------------
...
function tau = inv_dyn_1_FHG(traj_data, d_Ns, tau_c_Nm, tau_s_Nm, nu_ps, epsilon)

    % Systemparameter
    J_kgm2 = 0.06;  % hier etwa J1 + J2, damit der Motor beide beschleunigt

    q1_p = traj_data.q1_p;
    q1_pp = traj_data.q1_pp;

    % reibungskräfte berechnen (jeweils mit doppelten Geschwindigkeiten)
    tau_f = (2*tau_c_Nm + 2*tau_s_Nm .* exp(-q1_p.^2 ./ nu_ps)) .* tanh(epsilon.*q1_p) + 2*d_Ns .* q1_p;

    % Auswertung inverse Dynamik
    tau = J_kgm2.*q1_pp + tau_f;

end

% Dies ist die Zustandsraumdarstellung des Torsionsschwingers
function x_p = ZRD_2_FHG(t, x, tau_vec, t_vec, J_1_kgm2, J_2_kgm2, c_phi_Nmprad, d_Ns, tau_c_Nm, tau_s_Nm, nu_ps, epsilon)
    ...
    tau_f = [(tau_c_Nm + tau_s_Nm .* exp(-q_p(1).^2 ./ nu_ps)) .* tanh(epsilon.*q_p(1)) + d_Ns .* q_p(1);
        (tau_c_Nm + tau_s_Nm .* exp(-q_p(2).^2 ./ nu_ps)) .* tanh(epsilon.*q_p(2)) + d_Ns .* q_p(2)];

    q_pp = inv(M)*([tau; 0] - K*q - tau_f);
...
d_Ns = 0.02;    % Dämpung
tau_c_Nm = 0.3;   % Coulomb-Reibkraft
tau_s_Nm = 0.15;   % zusätzlicher statischer Anteil
nu_ps = 0.3;

epsilon = 100;  % Parameter zur Regularisierung mit tanh()
...
for i = 1:number_runs
    ...
    traj_data(i).tau = inv_dyn_1_FHG(traj_data(i), d_Ns, tau_c_Nm, tau_s_Nm, nu_ps, epsilon);
...
for i = 1:number_runs
    ...
    odefun = @(t, x) ZRD_2_FHG(t, x, tau_vec, t_vec, J_1_kgm2, J_2_kgm2, c_phi_Nmprad, d_Ns, tau_c_Nm, tau_s_Nm, nu_ps, epsilon);
    ...
    for j = 1:length(t_vec)
        ...
        % ZRD auswerten
        x_p_j = ZRD_2_FHG(t_vec(j), x_j, tau_vec, t_vec, J_1_kgm2, J_2_kgm2, c_phi_Nmprad, d_Ns, tau_c_Nm, tau_s_Nm, nu_ps, epsilon);
    ...
    % Reibkräfte berechnen
    tau_f = [(tau_c_Nm + tau_s_Nm .* exp(-x(:, 3).^2 ./ nu_ps)) .* tanh(epsilon.*x(:, 3)) + d_Ns .* x(:, 3), (tau_c_Nm + tau_s_Nm .* exp(-x(:, 4).^2 ./ nu_ps)) .* tanh(epsilon.*x(:, 4)) + d_Ns .* x(:, 4)];
    ...
    sim_data(i).tau_f_1 = tau_f(:, 1);
    sim_data(i).tau_f_2 = tau_f(:, 2);
...
Mass_Cor_test = [sim_data(test_idx(1)).M_11, sim_data(test_idx(1)).M_12, sim_data(test_idx(1)).M_22, sim_data(test_idx(1)).C_1, sim_data(test_idx(1)).C_2, sim_data(test_idx(1)).g_1, sim_data(test_idx(1)).g_2, sim_data(test_idx(1)).tau_f_1, sim_data(test_idx(1)).tau_f_2];
...
for i = test_idx(2:end)
    ...
    Mass_Cor_test = [Mass_Cor_test;
        sim_data(i).M_11, sim_data(i).M_12, sim_data(i).M_22, sim_data(i).C_1, sim_data(i).C_2, sim_data(i).g_1, sim_data(i).g_2, sim_data(i).tau_f_1, sim_data(i).tau_f_2];
end
...
Mass_Cor_training = [sim_data(training_idx(1)).M_11, sim_data(training_idx(1)).M_12, sim_data(training_idx(1)).M_22, sim_data(training_idx(1)).C_1, sim_data(training_idx(1)).C_2, sim_data(training_idx(1)).g_1, sim_data(training_idx(1)).g_2, sim_data(training_idx(1)).tau_f_1, sim_data(training_idx(1)).tau_f_2];
...
if savedata == true
    ...
    dateiName = 'SimData_Torsionsschwinger_friction_' + time_stamp + '_Samples_' + num_samples + '.mat';   
