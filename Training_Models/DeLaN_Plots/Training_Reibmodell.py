...
Datum:     29.07.2025
Beschreibung:
In diesem Skript wird das DeLaN modell mit den optimalen Hyperparametern (Studienarbeit) auf dem standard Datensatz Trainiert. Dies soll allgemein die Funktionalität von DeLaN zeigen.
'''
...
features_training, labels_training, _, _, _ = extract_training_data('Datensatz_Reibung_Nruns_37.mat', target_folder)  # Mein Modell Trainingsdaten
_, _, features_test, labels_test, Mass_Cor_test = extract_training_data('Datensatz_Reibung_Nruns_37.mat', target_folder)  # Mein Modell Testdaten (Immer dieselben Testdaten nutzen)
...
if just_plot == False:
    ...
    quad_subplot(
        ...
        np.concatenate([tau_fric_test[:, 0].reshape(-1, 1), Mass_Cor_test[:, 7].reshape(-1, 1)], axis=1).reshape(-1, 2),
        np.concatenate([tau_hat_test[:, 1].reshape(-1, 1), labels_test[:, 1].reshape(-1, 1)], axis=1).reshape(-1, 2), 
        np.concatenate([tau_fric_test[:, 1].reshape(-1, 1), Mass_Cor_test[:, 8].reshape(-1, 1)], axis=1).reshape(-1, 2)],
        r'Samples',
    ...
    save_path = os.path.join(script_path, 'Ergebnisse_Training_Reibmodell.npy')

    np.save(save_path, results)

else:

    data_path = os.path.join(script_path, 'Ergebnisse_Training_Reibmodell.npy')
